 ...
 * a button with <code>onRollOver</code> and <code>onRollOut</code> handlers that tween an object's _alpha and the user rolls their mouse
 * over/out/over/out quickly. Most likely, you'd want each new tween to overwrite the other immediately so
 * that you don't end up with multiple tweens vying for control of the _alpha property. That describes
 ...
 * 
 * 		TweenMax.to(mc, 1, {_x:100, overwrite:OverwriteManager.PREXISTING});<br />
 * 		TweenMax.to(mc, 1, {_x:100, overwrite:5});<br /><br /></code>
 * 
 ...
 * 		TweenLite.to(mc, 2, {_x:300});<br /><br /></code>
 * 		
 ...
 * 		TweenLite.to(mc, 2, {_x:"300", _y:"100"});<br />
 * 		TweenLite.to(mc, 1, {_alpha:50, overwrite:1}); //or use the constant OverwriteManager.ALL_IMMEDIATE<br />
 * 		TweenLite.to(mc, 3, {_x:200, _rotation:30, overwrite:2}); //or use the constant OverwriteManager.AUTO<br /><br /></code>
 * 		
 * 		
...
class com.greensock.OverwriteManager {
		...
		public static var ALL_IMMEDIATE:Number 	= 1;
		...
		public static var ALL_ONSTART:Number 	= 4;
		...
		public static var PREEXISTING:Number 	= 5;
		 ...
		 **/
		public static function init(defaultMode:Number):Number {
		 ...
		 **/
		public static function manageOverwrites(tween:TweenLite, props:Object, targetTweens:Array, mode:Number):Boolean {
			var i:Number, changed:Boolean, curTween:TweenLite;
			if (mode >= 4) {
				var l:Number = targetTweens.length;
				for (i = 0; i < l; i++) {
			...
			var startTime:Number = tween.startTime, overlaps:Array = [], cousins:Array = [], cCount:Number = 0, oCount:Number = 0;
			...
			if (mode == 2) {
				...
				while (i--) {
					if (overlaps[i].setEnabled(false, false)) { //flags for garbage collection
						changed = true;
					}
				}
			}
