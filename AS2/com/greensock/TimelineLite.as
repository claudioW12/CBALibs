...
class com.greensock.TimelineLite extends SimpleTimeline {
		...
		public function addChild(tween:TweenCore):Void {
			...
			if (first == undefined) {
				...
				while (curTween != undefined && st <= curTween.cachedStartTime) {
		 ...
		 */
		public function insert(tween:TweenCore, timeOrLabel:Object):Void {
		 ...
		 */
		public function append(tween:TweenCore, offset:Number):Void {
			insert(tween, this.duration + (Number(offset) || 0));
		}
		 ...
		 */
		public function prepend(tween:TweenCore, adjustLabels:Boolean):Void {
		 ...
		 */
		public function insertMultiple(tweens:Array, timeOrLabel:Object, align:String, stagger:Number):Void {
			if (timeOrLabel == undefined) {
				timeOrLabel = 0;
			}
			if (align == undefined) {
		 ...
		 * @param stagger staggers the tweens by a set amount of time (in seconds) (or in frames for frames-based timelines). For example, if the stagger value is 0.5 and the "align" property is set to TweenAlign.START, the second tween will start 0.5 seconds after the first one starts, then 0.5 seconds later the third one will start, etc. If the align property is TweenAlign.SEQUENCE, there would be 0.5 seconds added between each tween. Default is 0.
		 */
		public function appendMultiple(tweens:Array, offset:Number, align:String, stagger:Number):Void {
			insertMultiple(tweens, this.duration + (Number(offset) || 0), align, stagger);
		}
		 ...
		 * @param stagger staggers the tweens by a set amount of time (in seconds) (or in frames for frames-based timelines). For example, if the stagger value is 0.5 and the "align" property is set to TweenAlign.START, the second tween will start 0.5 seconds after the first one starts, then 0.5 seconds later the third one will start, etc. If the align property is TweenAlign.SEQUENCE, there would be 0.5 seconds added between each tween. Default is 0.
		 */
		public function prependMultiple(tweens:Array, align:String, stagger:Number, adjustLabels:Boolean):Void {
		...
		public function getChildren(nested:Boolean, tweens:Boolean, timelines:Boolean, ignoreBeforeTime:Number):Array {
			...
			var a:Array = [], tween:TweenCore = _firstChild || _endCaps[0], cnt:Number = 0;
			while (tween) {
		 ...
		 */
		public function killTweensOf(target:Object, nested:Boolean):Boolean {
			if (nested == undefined) {
				nested = true;
			}
...
//---- GETTERS / SETTERS -------------------------------------------------------------------------------------------------------
		...
		public function get totalDuration():Number {
			if (this.cacheIsDirty) {
				var max:Number = 0, end:Number, tween:TweenCore = _firstChild || _endCaps[0], prevStart:Number = -999999999999, next:TweenCore;
				while (tween) {
