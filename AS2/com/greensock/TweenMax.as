...
class com.greensock.TweenMax extends TweenLite {
		/** @private **/
		public static var version:Number = 11.2;
		...
		private function init():Void {
			...
			if (this.vars.roundProps && TweenLite.plugins.roundProps) {
				var j:Number, prop:String, multiProps:String, rp:Array = this.vars.roundProps, plugin:Object, ptPlugin:PropTween, pt:PropTween;
		...
		public function renderTime(time:Number, suppressEvents:Boolean, force:Boolean):Void {
			...
			if (_repeat != 0) {
				...
				if (isComplete) {
					...
					if (_cyclesComplete != (_cyclesComplete = Math.floor(this.cachedTotalTime / cycleDuration))) {
						repeated = true;
					}
